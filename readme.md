...
`$ php -f app.php`<br>
Использование консоли является предпочтительным, потому что при достаточно больших объёмах изображений приложению потребуется время для решения задачи. Так же лучше иметь дамп директорий, где планируется проводится работы. 